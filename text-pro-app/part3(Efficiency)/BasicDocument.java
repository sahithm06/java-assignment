...
public class BasicDocument extends Document 
{
	...
	public int getNumSentences()
	{
		...
		int count=0;
		String rgx="[^.!?]+" ;
		   Matcher m=Pattern.compile(rgx).matcher(getText());
		   while(m.find())
		   {
			  count++; 
		   }
		//System.out.println("The sentence on which we are perfroming the test is:\n");
        return count;
	}
	...
	public int getNumSyllables()
	{
		...
		List<String> list=getTokens("[a-zA-Z]+");
		int count=0;
		//System.out.println("the words and the syllables they have");
		for(int i=0;i<list.size();i++)
		{
			
		    count+=countSyllables(list.get(i));
		   // System.out.println(list.get(i)+" : "+countSyllables(list.get(i)));
